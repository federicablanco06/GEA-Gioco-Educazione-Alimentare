...
L'architettura software prevede la comunicazione di tre moduli principali: l'applicazione \acs{gea}, con cui il terapista (nella scelta iniziale del livello di difficoltà) comunica, che è collegata al programma di gestione di orientamento visivo (necessario per captare i movimenti e il focus del bambino) del \acs{vr} e che trasferisce i dati, grazie all'uso della applicazione Google Home, al Google Chromecast connesso al \acs{pc} del terapista che può quindi monitorare l'avanzamento del gioco e dare eventuali suggerimenti al paziente.
\vspace{70pt}
\begin{figure}[htbp]
\centering
